...
import { LuBaby, LuBrainCircuit, LuFileText, LuMessageSquare } from 'react-icons/lu';
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {products.map((product, index) => (
            <Card 
              key={index} 
              className={`overflow-hidden group transition-all duration-300 ${activeProduct === index ? 'ring-2 ring-indigo-500 shadow-xl' : 'hover:shadow-lg'}`}
            ...
            >
              <div className="relative h-48 overflow-hidden">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent transition-opacity">
                  <div className="absolute bottom-0 left-0 p-4">
                    <span className={`inline-flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-br ${product.color} text-white shadow-lg`}>
                      {product.icon}
                    </span>
                  </div>
                </div>
              ...
              <CardHeader>
                <h3 className="text-xl pt-5 font-semibold text-slate-900 dark:text-white">{product.title}</h3>
              </CardHeader>
